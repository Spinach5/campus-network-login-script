var AuthInterFace = (function() {
	...
	return {
		...
		loginOfCas : function(userId, flag, service, queryString,operatorPwd,operatorUserId,passwordEncrypt,rememberService,callback) {
			var content = "userId=" + userId + "&flag=" + flag + "&service=" + service + "&queryString=" + queryString+"&operatorPwd="+operatorPwd+"&operatorUserId="+operatorUserId+"&passwordEncrypt="+passwordEncrypt+"&rememberService="+rememberService;
			post(ePortalUrl + "loginOfCas", content, callback);
		},
		logoutForSession:function(userId,callback){
			var content = "";
			post(ePortalUrl+"logoutForSession",content,callback);
		},
        queryErrorInfo : function(code,callback) {
            /* 查询错误信息 */
            var content = "code=" + code;
            post_sync(ePortalUrl + "queryErrorInfo", content, callback);
        },
		portalAuthSuccessConfig : function(callback){
		 // 是否【启用Portal认证成功页配置】
		    post(ePortalUrl + "portalAuthSuccessConfig", "", callback);
		}
